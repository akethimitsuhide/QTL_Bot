...
### GIS地図描画機能（試験導入、2026-09-13〜。津波対応・自動ズーム・視認性向上は2026-09-14〜、遠地地震（国土地理院タイル重ね合わせ）対応は2026-09-15〜、観測点拡大表示・重なり順・気象庁形式の津波通知対応は2026-09-16〜、震度速報の区域アイコン・不透明色塗り・観測点マーカー拡大も2026-09-16〜、陸地／海の色分け・アンチエイリアスは2026-09-17〜）
気象庁のシェープファイル形式のGISデータ（有志によりGeoJSON化されたもの、CC-BY-4.0）を用いて、震源・震度分布・津波予報区等の地図画像を独自に描画する機能。旧・P2P地図画像添付機能の置き換え。データ取得・キャッシュは`core/gis_data.py`、日本国内向けの描画は`core/gis_render.py`（Pillowのみ使用。新規の重量級依存は追加していない）、震源が日本国外の場合の国土地理院タイル重ね合わせは`core/gis_tile_render.py`が担当する。`EewCog`・`QuakeInfoCog`・`TsunamiCog`に統合済み。
...
いずれも[Ichihai1415/JMA-GIS-GeoJSON](https://github.com/Ichihai1415/JMA-GIS-GeoJSON)（GeoJSON3種）および気象庁公式（stations.json）から取得する。加えて、遠地地震向けの国土地理院タイル（下記）は`GIS_MAP_DATA_DIR/tiles/`配下に取得のたびキャッシュする（静的な地図タイルのため無期限キャッシュ。これも`.gitignore`対象）。
...
- 津波情報: `notify_tsunami`（P2P経由）に加え、気象庁配信の`notify_tsunami_forecast`（津波予報・警報）・`notify_tsunami_observation`（津波観測情報）でも同様にGIS地図を添付する（2026-09-16追加。当初`notify_tsunami`にしか配線しておらず、気象庁形式の通知では地図が添付されない不具合があったため修正）。津波予報区の沿岸線を警報種別の色（`TSUNAMI_COLOR_*`。下記「津波色設定」参照）で塗り分け。津波情報解除（cancelled）の場合は塗るべき対象が無いため描画自体を行わない
- PLUM法（仮定震源要素）の場合は、震源のバツ印の代わりに`GIS_MAP_WARNING_COLOR`のドーナツ型の円を描画（警報・予報のいずれの場合も）
- 塗りつぶし区域は半透明フィル＋不透明の輪郭線の二重描画とし、隣接区域との境界が塗りつぶし後も判別できるようにしている（境界線の色は2026-09-15に濃く調整）
- 震源のバツ印・PLUMドーナツは、赤系の塗りつぶしに紛れて見えにくくならないよう、白い丸フチ（グレーの輪郭付き）を下地に敷いてから描画する（2026-09-14追加）

**遠地地震（震源が日本国外）向けの国土地理院タイル重ね合わせ（2026-09-15追加）**: 地震情報の震源座標が日本の表示範囲外（`core.gis_render.is_outside_japan_bbox()`で判定）の場合、`core.gis_render`の日本限定ベクター地図では震源位置を表現できないため、国土地理院の淡色地図タイル（`https://cyberjapandata.gsi.go.jp/xyz/pale/{z}/{x}/{y}.png`）を背景に、日本の細分区域境界線と震源のバツ印を重ね描きした画像に切り替わる（`core/gis_tile_render.py`）。
...
- タイルは`GIS_MAP_DATA_DIR/tiles/`に永続キャッシュ。取得に失敗したタイル（国土地理院側が日本国外のデータを持たない場合等）は空白のまま処理を継続し、区域線・震源マークは影響を受けない
...
**陸地／海の色分け・アンチエイリアス（2026-09-17追加）**: 従来は「塗られていない区域＝背景と同じ薄灰色」だったため日本列島の形そのものが把握しにくかったが、区域データを陸地色（クリーム色）で塗り、背景を海色（薄い水色）にすることで列島の形をひと目で分かるようにした。加えて、実際のサイズの2倍でキャンバスに描画してから最後にLANCZOSで縮小する簡易アンチエイリアス（スーパーサンプリング）により、県境・海岸線のギザつきを滑らかにしている。**この2つは`core/gis_render.py`内の独自ベクター地図（`render_eew_warn_map`/`render_shindo_map`/`render_tsunami_map`）専用で、国土地理院タイルとの重ね合わせ（`core/gis_tile_render.py`、遠地地震向け）には意図的に適用していない**（あちらは海外を含む実際の地図タイルを背景に使っており、本モジュールが持つ区域ポリゴンは日本国内分のみのため、陸地／海の色分けを適用すると台湾やカムチャツカ半島等の海外の陸地が誤って海のように表示されてしまう。台湾付近の震源でのテストでも、GSIタイル側が影響を受けないことを確認済み）。
...
外部データ（GeoJSON3種・観測点一覧）が未取得の場合、`GIS_MAP_ENABLE=true`でのCogロード時に自動ダウンロードする（初回のみ）。手動で強制再取得したい場合は `python3 bot.py --refresh_gis_data` を使う（stations.jsonの観測点構成が気象庁側で更新された場合など）。
...
**最終更新**: 2026-09-17（GIS地図描画（`core/gis_render.py`の独自ベクター地図）に陸地／海の色分け（陸地はクリーム色、海は水色）とアンチエイリアス（2倍スーパーサンプリング→LANCZOS縮小）を追加。国土地理院タイルとの重ね合わせ（`core/gis_tile_render.py`、遠地地震向け）は実際の地図タイルを背景に使っており区域ポリゴンは日本国内分のみのため、陸地／海の色分けは意図的に適用せず（適用すると台湾・カムチャツカ半島等の海外の陸地が誤って海のように表示されるため）、台湾付近の震源でも従来通り正しく表示されることをテストで確認済み)
**対応 Python**: 3.11+
