...
### P2P 地図画像の添付
- 地震情報・津波情報・EEW（P2P由来）の通知には、P2P 地震情報 CDN が生成する震源地図画像を Embed に添付する（`core/p2p_image.py` の `P2PImageMixin`。`QuakeInfoCog` / `TsunamiCog` / `EewCog` が多重継承）。地震感知情報（`JishinKanchiCog`）は2026-09-11以降、画像取得を行わない（上記「P2P 地震感知情報」参照）
...
- **最終失敗時の診断ログ強化（2026-09-09追加）**: 全20回のリトライに失敗した場合のログに、それまでは `id` のみを出力していたが、最終試行時点の詳細（HTTPステータスコード、またはPNG構造として不正だった場合はそのバイト数、例外の場合はその内容）を追記した。「CDN側でそもそも画像が生成されない（恒常的にHTTP 404）」のか「生成が遅いだけでいずれ成功する」のかを、DEBUGログを有効化しなくても運用中のINFOログから判別できるようにするための変更。実際にこのログを使って、地震感知情報（`JishinKanchiCog`）の画像取得が常にHTTP 404で失敗することを特定できた（上記「P2P 地震感知情報」参照）
