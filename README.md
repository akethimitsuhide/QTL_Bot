...
- 地震情報（震度速報以外で観測点情報がある場合）: 観測点ごとに震度を表す短いラベル（"1"〜"7"、5弱/5強は"5-"/"5+"、6弱/6強は"6-"/"6+"）付きの色付き四角マーカー（16px、2026-09-16に11pxから拡大）を配置。震度3以上の観測点がある場合はその周辺だけを拡大表示し（無ければ従来通り全観測点を表示範囲に含める）、観測点同士が重なる場合は震度が大きいものほど手前（前面）に描画する（2026-09-16追加）
