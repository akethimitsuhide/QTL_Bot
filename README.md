...
- 長周期地震動に関する観測情報（2026-09-17追加、観測点マップは2026-09-18追加）: JMAの`Coordinate`形式の座標文字列（`core.helpers.parse_jma_coordinate()`でパース）から震源の緯度経度を取り出したバツ印マップに加え、地震情報向けの観測点座標データ（stations.json）を使って観測点ごとの長周期地震動階級（"1"〜"4"）を色分けした地図（`render_long_period_map`、`LG_COLORS`使用。SHINDO_COLORSとは別スケールのため専用関数にしている）を2枚目として添付する
...
- 国土地理院コンテンツ利用規約に基づき、画像右下に「出典：国土地理院」の出典表示を必ず入れる（日本語フォントが見つからない環境では英語表記"Source: GSI, Japan"にフォールバックする）
