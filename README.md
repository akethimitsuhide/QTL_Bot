...
### GIS地図描画機能（試験導入、2026-09-13〜。津波対応・自動ズーム・視認性向上は2026-09-14〜、遠地地震（国土地理院タイル重ね合わせ）対応は2026-09-15〜、観測点拡大表示・重なり順・気象庁形式の津波通知対応は2026-09-16〜）
...
- 地震情報（震度速報以外で観測点情報がある場合）: 観測点ごとに震度を表す短いラベル（"1"〜"7"、5弱/5強は"5-"/"5+"、6弱/6強は"6-"/"6+"）付きの色付き四角マーカーを配置。震度3以上の観測点がある場合はその周辺だけを拡大表示し（無ければ従来通り全観測点を表示範囲に含める）、観測点同士が重なる場合は震度が大きいものほど手前（前面）に描画する（2026-09-16追加）
- 津波情報: `notify_tsunami`（P2P経由）に加え、気象庁配信の`notify_tsunami_forecast`（津波予報・警報）・`notify_tsunami_observation`（津波観測情報）でも同様にGIS地図を添付する（2026-09-16追加。当初`notify_tsunami`にしか配線しておらず、気象庁形式の通知では地図が添付されない不具合があったため修正）
- 津波情報（2026-09-14追加）: 津波予報区の沿岸線を警報種別の色（`TSUNAMI_COLOR_*`。下記「津波色設定」参照）で塗り分け。津波情報解除（cancelled）の場合は塗るべき対象が無いため描画自体を行わない
...
**最終更新**: 2026-09-16（**不具合修正**：気象庁配信の津波予報・警報（`notify_tsunami_forecast`）・津波観測情報（`notify_tsunami_observation`）にGIS地図描画の配線が入っておらず、地図が添付されない不具合を修正（P2P経由の`notify_tsunami`にしか配線していなかった。実機テストで発覚）／観測点マーカーの表示範囲を、震度3以上の観測点があればその周辺だけに拡大表示するよう変更（無ければ従来通り全観測点を含める）／観測点マーカーが重なる場合、震度が大きいものほど手前（前面）に描画されるよう描画順を震度コード昇順に変更）
**対応 Python**: 3.11+
