...
### GIS地図描画機能（試験導入、2026-09-13〜。津波対応・自動ズーム・視認性向上は2026-09-14〜、遠地地震（国土地理院タイル重ね合わせ）対応は2026-09-15〜）
...
**最終更新**: 2026-09-15（**不具合修正**：`GIS_MAP_ENABLE=true`でも外部データのダウンロードが一度も実行されず地図が添付されない不具合を修正（`EewCog`/`QuakeInfoCog`/`TsunamiCog`の`cog_load()`で`core.gis_data.ensure_gis_data_ready()`を呼ぶよう修正。データ未取得時のログもDEBUGからWARNINGへ引き上げ）／区域境界線を濃く調整／遠地地震（震源が日本国外）向けに、国土地理院の淡色地図タイルと日本の区域境界線を重ね合わせて描画する`core/gis_tile_render.py`を追加し`QuakeInfoCog`に統合（Web Mercator投影、日付変更線をまたぐ震源〈南米・北米西岸等〉でも太平洋を挟んだ正しい近い側の範囲を選択、出典表示「出典：国土地理院」付き、タイルは`GIS_MAP_DATA_DIR/tiles/`に永続キャッシュ））
**対応 Python**: 3.11+
