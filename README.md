...
- **P2P EEW（緊急地震速報（警報）専用）**: P2P 地震情報の WebSocket から警報のみを常時受信
  ...
  - **【2026-09-13 廃止】** 以前はP2Pメッセージの `id` から地図画像URLを組み立ててEmbed最下部に添付していたが、気象庁シェープファイル/GeoJSONベースの独自地図描画機能（試験導入予定）に置き換えるため廃止した
  - **PLUM法（震源要素を経由せず、観測波形から直接各地点の震度を推定する気象庁の緊急地震速報手法）による予測時、マグニチュード・深さが「推定なし」になるのは仕様通り**。PLUM法はそもそも震源の規模・位置を推定しないため、マグニチュード・深さが不明のまま、震度だけが観測波形から直接推定される。震源が未確定でも震度は判明する、という一見矛盾した組み合わせがPLUM法の正常な出力（「推定最大震度7以上」のような表示は、後述の**2026-09-22修正**より前の誤った実装によるものであり仕様ではない）
...
- **地震情報**: P2P 地震情報の API からの確報情報
  ...
  - **【地域ごとの予想震度】の下限値／上限値のどちらかが不明なエリアの表示（2026-09-02修正、2026-09-22に前提を訂正）**: EEW（P2P生形式）のエリアには、推定精度上の理由で震度の下限（`scaleFrom`）と上限（`scaleTo`）のうち一方だけが判明しているケースがある。以前は下限（`Shindo1`）が「不明」のエリアを地域内訳から丸ごと除外していたため、全体の「予想最大震度」（`MaxIntensity`）とは食い違う不整合が生じていた。下限・上限のうち少なくとも一方が判明していれば表示するよう`core/eew_convert.py`の`build_forecast_groups`を修正した（単独EEW通知・複数EEWサマリー通知の両方に適用される共通関数）
  - **P2P地震情報のエリアの`scaleTo=99`は「震度7」ではなく「以上」＝上限なしを表す（2026-09-22修正）**: 気象庁電文の`To="over"`に相当する値で、`scaleFrom=45, scaleTo=99`は「震度5弱**以上**」（PLUM法では各地の推定がこの形式になりやすい）という意味。以前は`scaleTo=99`を一律「震度7」として扱っていたため、PLUM法で全地域が「5弱以上」と推定されているだけなのに、全体の予想最大震度（`MaxIntensity`）が実態と無関係に「7以上」と表示され、地域ごとの内訳（5弱・5強等）とも食い違う不具合があった。修正後は「以上」を`core.eew_convert.OPEN_UPPER_LABEL`として扱い、全体の予想最大震度・地域ごとの内訳・地図の塗り色のいずれも下限値ベースで一貫して「◯◯以上」と表示する（`Shindo1`が不明かつ`Shindo2`が「以上」のみのエリアは、震度の手掛かりが無いため地域内訳・地図には出さない）。Wolfx形式（`Shindo2`が電文由来の`"over"`表記のケース）も同様に扱う（`core/helpers.py`の`shindo_code_from_max_label`で、Embedの色・警戒喚起文・音声判定・地図描画のすべてが「◯◯以上」表記を取りこぼさないようにしている）
  - **【2026-09-13 廃止】** 以前はメッセージ送信後にバックグラウンドで P2P 地震情報 CDN をポーリングし、地図画像を Embed に追加していたが、気象庁シェープファイル/GeoJSONベースの独自地図描画機能（試験導入予定）に置き換えるため廃止した
...
- **地図画像は添付しない（2026-09-11確定・廃止）**: 以前はP2P地震情報通知と同じ生成方法（`cdn.p2pquake.net/app/images/{id}_trim_big.png`。`_id`優先→`id`フォールバック、2026-08-27修正）でEmbed最下部に地図画像を添付していたが、実機ログで4件の地震感知情報イベントを検証した結果、対応する画像が毎回・例外なく20回リトライ（約2分間）後にHTTP 404で取得失敗することが判明した。「生成が遅いだけ」であれば偶発的に成功する試行が見られるはずだが皆無だったため、地震感知情報（code=9611）の`_id`に対応する地図画像はP2P地震情報API・CDN側にそもそも用意されていない（仕様上の制約）と判断し、画像取得の試行自体を廃止した（無駄なリトライでCDN共有セマフォ枠を専有し続けることも避けられる）
...
- **音声トリガーは第一報のみ（2026-08-23追加、2026-08-27仕様変更）**: 同一イベント（`started_at` で識別。EEWのEventIDに相当）について更新が届くたびに音声読み上げ・効果音が毎回鳴ると煩わしいため、音声読み上げ・効果音とも「そのイベントを初めて検知したとき（＝第一報、EEWの第一報相当）」の1回のみ再生する。以前は読み上げのみ件数（`count`）増加ごとに再トリガーしていたが、繰り返し鳴ってうるさいとの指摘を受けて統一した
  - **（過去の経緯・2026-09-11に不要化）地図画像の添付タスクも第一報のみに間引く対策（2026-08-30追加）**: 感知報告が増えるたびにP2P側から `_id` が異なる新しい更新レコードが配信され続けるため、従来は更新のたびに新規の画像取得タスクを起動していた。実機ログで、関東で広く感知された地震において短時間に70件近い異なる `_id` で画像取得タスクが同時多発し、`P2P_IMAGE_CDN_CONCURRENCY`（下記「P2P 地図画像の添付」参照）のセマフォを占有し尽くし、`QuakeInfoCog` 側の正当な画像取得まで巻き添えで大幅に遅延・失敗させる不具合を確認したため、画像添付タスクの起動可否も「第一報かどうか」の判定に統一する対策を行った。その後2026-09-11に、第一報の画像取得自体が常にHTTP 404で失敗することが判明し、画像取得機能自体を廃止したため、この対策も不要になった（テキスト通知・音声・効果音の「第一報のみ」判定は現在も有効）
...
### GIS地図描画機能（試験導入、2026-09-13〜。津波対応・自動ズーム・視認性向上は2026-09-14〜、遠地地震（国土地理院タイル重ね合わせ）対応は2026-09-15〜、観測点拡大表示・重なり順・気象庁形式の津波通知対応は2026-09-16〜、震度速報の区域アイコン・不透明色塗り・観測点マーカー拡大も2026-09-16〜、陸地／海の色分け・アンチエイリアス・震源マークデザイン変更・国境データ重ね合わせ・対応情報種別拡大は2026-09-17〜、USGS対応・長周期地震動の観測点マップ・通常表示の併記・Webダッシュボード連携は2026-09-18〜）
気象庁のシェープファイル形式のGISデータ（有志によりGeoJSON化されたもの、CC-BY-4.0）を用いて、震源・震度分布・津波予報区等の地図画像を独自に描画する機能。旧・P2P地図画像添付機能の置き換え。データ取得・キャッシュは`core/gis_data.py`、日本国内向けの描画は`core/gis_render.py`（Pillowのみ使用。新規の重量級依存は追加していない）、震源が日本国外の場合の国土地理院タイル重ね合わせは`core/gis_tile_render.py`、1通知に複数のGIS地図画像を添付する際のDiscord埋め込み組み立ては`core/gis_discord.py`が担当する。`EewCog`・`QuakeInfoCog`・`TsunamiCog`・`OtherInfoCog`（長周期地震動・顕著な地震の震源要素更新のお知らせ）・`UsgsCog`（2026-09-18追加）に統合済み。

> **【2026-09-15 不具合修正】** `GIS_MAP_ENABLE=true`にしても、各Cogから外部データのダウンロードを実行する配線が入っておらず、`data/gis/`ディレクトリ自体が作成されない＝地図が一切添付されない不具合があった。各Cogの`cog_load()`で`core.gis_data.ensure_gis_data_ready()`を呼び出すよう修正した（詳細は変更履歴参照）。また、外部データ未取得時のログを`DEBUG`から`WARNING`に引き上げ、原因が分かりやすいようにした。

**有効化**: `.env`で`GIS_MAP_ENABLE=true`にする（デフォルト無効。試験導入のためオプトイン方式）。有効化すると、各Cogロード時に必要な外部データが未取得であれば自動的にダウンロードする（初回のみ。手動で強制再取得したい場合は`python3 bot.py --refresh_gis_data`）。
...
- `AreaForecastLocalE_GIS`（細分区域、194件。緊急地震速報（予報）・全地震情報の震度分布描画用。区域名はREGION_MAP変換前の生の名称と一致。2件はgeometryがnull＝境界データ未整備のため塗りつぶし対象外。遠地地震時の国土地理院タイル重ね合わせでも、日本の輪郭を示す区域線としてこのデータを再利用する）
- `AreaTsunami_GIS`（津波予報区、70件。津波情報の沿岸線描画用。2026-09-14追加。geometryはPolygonではなくLineString/MultiLineString＝海岸線に沿った線データで、気象庁の実際の津波予報区表示（塗りつぶしではなく色付き沿岸線）に合わせている。区域名は津波情報APIの`areas[].name`と一致。4件はgeometryがnull＝「帰属未定」のプレースホルダーのため実データで参照されることはない）
- `stations.json`（気象庁 震度観測点一覧。観測点ごとの震度マーカー描画の緯度経度取得用）
- `countries.geojson`（[datasets/geo-countries](https://github.com/datasets/geo-countries)、258カ国、日本を除く。遠地地震向けの国土地理院タイル重ね合わせで、タイルが薄い海外の陸地を補う下地用。2026-09-17追加）

いずれも[Ichihai1415/JMA-GIS-GeoJSON](https://github.com/Ichihai1415/JMA-GIS-GeoJSON)（GeoJSON3種）・[datasets/geo-countries](https://github.com/datasets/geo-countries)（countries.geojson）・気象庁公式（stations.json）から取得する。加えて、遠地地震向けの国土地理院タイル（下記）は`GIS_MAP_DATA_DIR/tiles/`配下に取得のたびキャッシュする（静的な地図タイルのため無期限キャッシュ。これも`.gitignore`対象）。

**描画内容**:
- 緊急地震速報（警報）: 発表地域（強い揺れが予想される地域。府県予報区単位）を`GIS_MAP_WARNING_COLOR`（デフォルト赤）で塗りつぶし＋震源にバツ印。地域ごとの予想震度（`WarnArea`）が取得できている場合、震度速報時と同じ塗り分けの地図（`render_shindo_map`）を2枚目として併せて添付する（**2026-09-22追加**。発表地域だけでは各地の震度の強弱が分からないとの指摘のため。地域別震度が無いEEWの場合は2枚目を省略する）
- 緊急地震速報（予報）・全地震情報（震度速報）: 地域ごとの予想/観測震度を`SHINDO_COLORS`（震度色設定。上記参照）で塗り分け＋震源にバツ印。塗りは半透明ではなく設定色そのまま（不透明）で描画し、区域境界は「塗り色より暗い色」の輪郭線で確保する（2026-09-16変更。半透明だと色が薄く見づらいとの指摘のため）
- 震度速報（ScalePrompt）: 上記の区域塗りに加え、塗りつぶしだけでは震度が分かりにくいとの指摘を受け、各区域の中央（重心）に震度ラベル付きの大きめアイコン（28px）を追加配置する（2026-09-16追加）
- 地震情報（震度速報以外で観測点情報がある場合）: 観測点ごとに震度を表す短いラベル（"1"〜"7"、5弱/5強は"5-"/"5+"、6弱/6強は"6-"/"6+"）付きの色付き四角マーカー（24px、2026-09-16に11px→16px→24pxへ拡大。S/M/Lの比較サンプルを提示し、Lの24pxを採用）を配置。震度3以上の観測点がある場合はその周辺だけを拡大表示し（無ければ従来通り全観測点を表示範囲に含める）、観測点同士が重なる場合は震度が大きいものほど手前（前面）に描画する（2026-09-16追加）。震度3以上で拡大表示になった場合、全観測点を見渡せる通常表示（`station_zoom_priority=False`）の画像も2枚目として併せて添付する（2026-09-18追加。`core/gis_discord.py`の`build_gis_message_kwargs()`で複数Embedとして送信）
- 津波情報: `notify_tsunami`（P2P経由）に加え、気象庁配信の`notify_tsunami_forecast`（津波予報・警報）・`notify_tsunami_observation`（津波観測情報・沖合の津波観測に関する情報）でも同様にGIS地図を添付する（2026-09-16追加。当初`notify_tsunami`にしか配線しておらず、気象庁形式の通知では地図が添付されない不具合があったため修正）。津波予報区の沿岸線を警報種別の色（`TSUNAMI_COLOR_*`。下記「津波色設定」参照）で塗り分け。津波情報解除（cancelled）の場合は塗るべき対象が無いため描画自体を行わない
- 長周期地震動に関する観測情報（2026-09-17追加、観測点マップは2026-09-18追加、座標データの取得元は2026-09-22修正）: JMAの`Coordinate`形式の座標文字列（`core.helpers.parse_jma_coordinate()`でパース）から震源の緯度経度を取り出したバツ印マップに加え、観測情報JSON自体（`Body.Intensity.Observation...IntensityStation[]`の`latlon`）に含まれる観測点座標を使って観測点ごとの長周期地震動階級（"1"〜"4"）を色分けした地図（`render_long_period_map`、`LG_COLORS`使用。SHINDO_COLORSとは別スケールのため専用関数にしている）を2枚目として添付する。**2026-09-22修正**: 従来は地震情報向けの観測点座標データ（stations.json）を「地域名（埼玉県南部 等）」で名前引きしていたが、渡していたのが観測点名ではなく地域名だったため1件も一致せず、観測点マップが常に震源のみの1枚目と同一の画像になっていた（観測点が1件も描画できない場合は、重複を避けるためこの2枚目自体を添付しない）
- 顕著な地震の震源要素更新のお知らせ（2026-09-17追加）: JMAの`Coordinate_WGS`形式の座標文字列から震源の緯度経度を取り出し、震源のバツ印のみ描画する
- USGS地震情報（2026-09-18追加）: USGSのGeoJSON標準形式（`[経度,緯度,深さ]`）の座標をそのまま使用。震源が日本国内／近海か国外かで通常のベクター地図・国土地理院タイル重ね合わせを自動的に切り替える
...
- EEW（緊急地震速報）の地図（`render_eew_warn_map`・`render_shindo_map`（`enlarge_hypocenter_mark=True`指定時）に限る）では、震源のバツ印が小さく見づらいとの指摘を受け、2026-09-22に表示範囲の広さに応じて1.5倍（日本全体表示時）〜2.0倍（最大ズーム時）に連続的に拡大するようにした（`_zoom_mark_scale`）。地震情報・長周期地震動・震源要素更新のお知らせの地図（震度速報等）は従来通りの大きさのまま

**遠地地震（震源が日本国外）向けの国土地理院タイル重ね合わせ（2026-09-15追加、国境データ重ね合わせは2026-09-17追加）**: 地震情報の震源座標が日本の表示範囲外（`core.gis_render.is_outside_japan_bbox()`で判定。震源の緯度経度が日本全体の固定バウンディングボックス〈およそ経度122〜155度・緯度23〜46度〉の範囲外かどうかを見るだけの単純な判定で、実際の国境線やJMAの発表種別コードは見ていない）の場合、`core.gis_render`の日本限定ベクター地図では震源位置を表現できないため、国土地理院の淡色地図タイル（`https://cyberjapandata.gsi.go.jp/xyz/pale/{z}/{x}/{y}.png`）を背景に、世界の国境データ・日本の細分区域境界線・震源のバツ印を重ね描きした画像に切り替わる（`core/gis_tile_render.py`）。この判定は`cogs/quake.py`（地震情報）・`cogs/other.py`（長周期地震動・顕著な地震の震源要素更新）の両方で使われている。
- 表示範囲は「日本全体 ∪ 震源」を収める最も詳細なズームレベル（タイル数上限64枚以内）を自動選択
- 日付変更線をまたぐ震源（南米・北米西岸等、太平洋を挟んで日本の反対側にある場合）でも、地球を逆回りした遠い側ではなく太平洋を挟んだ近い側の範囲を正しく選択する
- タイルは`GIS_MAP_DATA_DIR/tiles/`に永続キャッシュ。取得に失敗したタイルは透明のまま処理を継続する（下記の国境データ・海色が透けて見える）
- **世界の国境データの重ね合わせ（2026-09-17追加）**: 国土地理院タイルは日本国内データが中心で、震源の相手国（台湾・カムチャツカ半島等）の陸地についてはタイルが取得できない／内容が薄いことがある。そこで、[datasets/geo-countries](https://github.com/datasets/geo-countries)の世界の国境データ（`countries.geojson`、258カ国、日本を除く）を陸地色で塗りつぶした下地を、国土地理院タイルの背後に敷くようにした。レイヤー順は「海色の背景 → 国境データ（陸地色、日本を除く） → 国土地理院タイル（取得できた範囲のみ） → 日本の区域境界線（線のみ） → 震源マーク」。これにより、国土地理院タイルが薄い海外の地域でも陸地か海かが常に判別できる（日本自体は自前のGeoJSONの方が精密なため国境データからは除外し、実際の地図タイルをそのまま前面に見せる）
- 国土地理院コンテンツ利用規約に基づき、画像右下に出典表示を必ず入れる。地理院タイルに国境データ・区域境界線・震源マークを重ねて加工しているため、**2026-09-22修正**で「出典：国土地理院（地理院タイル〈淡色地図〉を加工して作成）」という加工の旨を含む表記に改めた（画像幅が足りない場合は「出典：国土地理院（加工して作成）」の短縮版に、日本語フォントが見つからない環境では英語表記"Source: GSI, Japan (pale tiles, modified)"にフォールバックする）
- 投影はこの機能のみWeb Mercator（GSIタイルの座標系に合わせるため。`core.gis_render`側の簡易正距円筒図法とは別実装）

**自動ズーム（2026-09-14追加）**: 描画対象（塗りつぶし区域・観測点・震源・津波予報区の沿岸線）の緯度経度から表示範囲を動的に計算し、対象が日本の一部にとどまる場合はその周辺だけを拡大して描画する（余白は表示範囲の35%、最小表示範囲は2度四方を確保）。対象が広範囲、または該当データが無い場合は日本全体表示にフォールバックする。表示範囲外の区域境界線は描画自体をスキップし、Raspberry Pi での負荷を抑えている。

**陸地／海の色分け・アンチエイリアス（2026-09-17追加）**: 従来は「塗られていない区域＝背景と同じ薄灰色」だったため日本列島の形そのものが把握しにくかったが、区域データを陸地色（クリーム色）で塗り、背景を海色（薄い水色）にすることで列島の形をひと目で分かるようにした。加えて、実際のサイズの2倍でキャンバスに描画してから最後にLANCZOSで縮小する簡易アンチエイリアス（スーパーサンプリング）により、県境・海岸線のギザつきを滑らかにしている。この2つ（塗りの実装・アンチエイリアスの実装そのもの）は`core/gis_render.py`内の独自ベクター地図（`render_eew_warn_map`/`render_shindo_map`/`render_tsunami_map`）専用で、国土地理院タイルとの重ね合わせ（`core/gis_tile_render.py`、遠地地震向け）には使っていない（投影方式がWeb Mercatorで異なる上、ラスター画像の縮小はベクター線の縮小とは事情が異なるため）。ただし陸地色・海色の配色自体は、`core/gis_tile_render.py`側でも「国土地理院タイルが取得できなかった海外の箇所を、世界の国境データ（`countries.geojson`）で補う下地」として再利用している（上記「遠地地震向けの国土地理院タイル重ね合わせ」参照。当初は海外の陸地データを持たずこの色分けを適用できなかったが、`countries.geojson`の追加により解消した）。

投影は緯度経度→ピクセルの簡易正距円筒図法（表示範囲の中心緯度でcos補正）で、`shapely`/`cartopy`/`matplotlib`等の追加依存は使わない。GeoJSONのパース結果（区域名→生の緯度経度リング＋bbox）はプロセス内で一度だけ行いメモリ上に保持する（ピクセルへの投影は表示範囲ごとに変わるため描画のたびに行うが、900×900px相当の描画で1回あたり数十〜100ms程度。アンチエイリアス導入後も0.2〜0.35秒程度）。`GIS_MAP_ENABLE=false`または外部データ未取得の場合は各描画関数が`None`を返し、画像添付自体が省略される（通知本体の送信は妨げない）。
...
- **通知経路側の遅延も特定し、こちらは修正した**: 従来は通知のたびに`jma_s`系統・`abrspmx_s`系統の画像URLを、現在時刻の`KYOSHIN_IMAGE_DELAY_SEC`（既定6）秒前から直列にHEADリクエストで探索し直しており、検知に実際に使った最新フレームを再利用していなかった。加えて振動レベルの取得も直列だった。**2026-09-22修正**: 検知（画像解析）に使った最新フレームのURLを（10秒以内のものであれば）そのまま通知に再利用し、`abrspmx_s`系統の画像探索と振動レベル取得は`asyncio.gather`で並列化した（`cogs/kyoshin_monitor.py`の`_send_kyoshin_image`、`core/kyoshin_shared.py`の`DualImageFetcher.fetch_lmoni_url`・`fetch_urls`の並列化）。検知イベントごとに「イベント生成→初回通知」の所要秒数・検出観測点数・フェーズ・解析画像の古さをINFOログに1回出力するようにしたので（`qtlbot.log`で`初回通知`を検索）、実機でどの要因がどれだけ効いているか継続して確認できる
...
- `GET /status/history` でシステムリソース・受信件数の推移スナップショット履歴を JSON で取得（`STATUS_HISTORY_INTERVAL` 秒ごとに記録、メモリ上のリングバッファのみで保持しBot再起動でリセットされる）。`?format=csv` を付けると同じデータを CSV（UTF-8 BOM付き）でダウンロード可能。受信件数の内訳（`recv_count`）には `jishin_kanchi`・`kyoshin` も含む（**2026-09-09修正**: `/dashboard` の受信件数推移グラフは元々この2種別も描画しようとしていたが、スナップショット側にキー自体が無く常に0（グラフ上でフラットな線）になっていた。単純な転記漏れだったため、スナップショットの構築側に追加して修正した）
- `GET /status/notifications` で実際にDiscordへ送信した通知（種別・タイトル・時刻）の履歴を JSON で取得。`?limit=N` で件数を絞り込み可能（省略時は`QUAKE_HISTORY_DEFAULT_LIMIT`）。**2026-09-09変更**: 以前はメモリ上のリングバッファ（最大50件、Bot再起動でリセット）のみだったが、地震情報・EEW履歴と同じ方式でqtlbot.logへも記録するようにし、再起動をまたいで永続化された履歴を返せるようにした（詳細は下記「通知履歴の永続化」参照）
...
- **地震情報履歴の地図・表閲覧（2026-09-06追加、2026-09-09にEEW対応・日付/震度絞り込み・月次件数グラフ対応、2026-09-18にGIS区域境界線オーバーレイ対応）**: `GET /quake_map` で、地震情報（P2P地震情報 code=551）・EEW（緊急地震速報）の通知履歴を地図（Leaflet + 国土地理院タイル、震度に応じた色分けマーカー）と表の両方で、ページ内の種別セレクタで切り替えながら閲覧できる。発生日（から〜まで）・最大震度でのフィルタ、月次件数の棒グラフ（Chart.js、フィルタ適用後の件数を表示）も備える。データ取得元は `GET /status/quake_history` / `GET /status/eew_history`（`?limit=N` で件数を絞り込み可能。省略時は `QUAKE_HISTORY_DEFAULT_LIMIT`）で、`core/quake_history_log.py` / `core/eew_history_log.py` が qtlbot.log（ローテーション含む全世代）をスキャンして復元した内容をそのまま返す。GIS地図描画機能でキャッシュ済みの細分区域GeoJSON（`GET /status/gis_local_areas`。`GIS_MAP_ENABLE=false`またはキャッシュ未取得時は404）があれば、地図上に区域境界線を薄く重ね描きする（無ければ何もせず従来通りマーカーのみの地図のまま動作する）。詳細は下記「地震情報履歴（qtlbot.logベース）」参照
- `!status` コマンド（管理者専用）
- `/qtl_status` スラッシュコマンド（管理者専用）

### 通知履歴の永続化（qtlbot.logベース、2026-09-09追加）
`core/notification_log.py`（全種別横断の「直近の通知履歴」）は、以前はメモリ上のリングバッファ（最大50件）のみで保持しており、Bot再起動でリセットされていた。地震情報・EEW履歴（下記）と同じ設計で、`record_notification()` の呼び出しごとに `NOTIF_RECORD_V1 {json}` 形式の1行をqtlbot.logへも記録するようにし、`load_notification_history()` でqtlbot.log*（ローテーション含む）から復元できるようにした。`GET /status/notifications` はこの永続化された履歴を返す（`QUAKE_HISTORY_CACHE_TTL_SEC` 秒キャッシュ）。インメモリのリングバッファ自体（`get_recent_notifications()`）はファイルI/Oなしの最速参照用途向けに残しているが、現時点でWeb Dashboard側はqtlbot.logベースの方を使う。
...
| `SHINDO_COLOR_1` | `0x3098BD` | 震度1のEmbed色 |
| `SHINDO_COLOR_2` | `0x4CD0A7` | 震度2のEmbed色 |
| `SHINDO_COLOR_3` | `0xF6CB51` | 震度3のEmbed色 |
| `SHINDO_COLOR_4` | `0xFF9939` | 震度4のEmbed色 |
| `SHINDO_COLOR_5_LOWER` | `0xE52A18` | 震度5弱のEmbed色 |
| `SHINDO_COLOR_5_UPPER` | `0xC31B1B` | 震度5強のEmbed色 |
| `SHINDO_COLOR_6_LOWER` | `0xA30A6B` | 震度6弱のEmbed色 |
...
### 津波色設定（Embed配色 / GIS地図描画で共通利用、2026-09-14〜）
| 変数名 | 既定値 | 説明 |
|:---|:---|:---|
| `TSUNAMI_COLOR_MAJOR_WARNING` | `0xD344FC` | 大津波警報のEmbed色・GIS地図の沿岸線色 |
| `TSUNAMI_COLOR_WARNING` | `0xF93022` | 津波警報のEmbed色・GIS地図の沿岸線色 |
| `TSUNAMI_COLOR_WATCH` | `0xEEDB2D` | 津波注意報のEmbed色・GIS地図の沿岸線色 |
| `TSUNAMI_COLOR_UNKNOWN` | `0x56BCFC` | 不明・その他のEmbed色・GIS地図の沿岸線色 |

表記は震度色設定と同じ。従来 `cogs/tsunami.py` にハードコードされていた色を集約したもの（デフォルト値は従来と同一）。
...
└── core/
    ...
    ├── gis_data.py                 - GIS地図描画（試験導入）用の外部データ（GeoJSON4種・
    │                                 stations.json）のダウンロード・キャッシュ管理
    ├── gis_render.py               - GIS地図描画（試験導入）のPillowベース描画エンジン
    │                                 （EEW警報地域・震度分布・観測点マーカー・津波予報区
    │                                 沿岸線の地図PNG生成。表示範囲の自動ズーム・陸地／海
    │                                 の色分け・アンチエイリアス対応）
    ├── gis_tile_render.py           - 遠地地震（震源が日本国外）向け。国土地理院タイル
    │                                 ＋世界の国境データ（countries.geojson）との重ね合わせ
    │                                 表示（Web Mercator投影、日付変更線またぎ対応）。
    │                                 core/gis_render.pyとは別の投影方式
    ├── gis_discord.py               - GIS地図画像（複数枚）をDiscordの複数Embedとして
    │                                 送信する際のkwargs組み立て（build_gis_message_kwargs）
