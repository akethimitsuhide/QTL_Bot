...
### 📝 ログローテーション機能（A-2）
- `RotatingFileHandler` による自動ログローテーション
- デフォルト: 10MB ごと、最大 7 ファイル保持
- 環境変数で設定可能（`LOG_MAX_BYTES`, `LOG_BACKUP_COUNT`）
- Windows / macOS / Linux すべての環境で動作

### 📊 リソース監視機能（A-5）
- 1 時間ごとに CPU・メモリ・ディスク使用率を記録
- ディスク使用率が 80% 以上で WARNING、90% 以上で ERROR を記録
- `psutil` ライブラリで各 OS に対応
- psutil がない環境では安全にスキップ
- 環境変数で監視間隔・閾値を設定可能
...
| `USGS_CHANNEL_ID` | - | USGS 通知専用チャンネル（未設定時は QUAKE_CHANNEL_ID） |
...
| `USGS_FETCH_INTERVAL` | 600 | ポーリング間隔（秒、デフォルト10分） |
| `USGS_REGION_LAT_MIN` | 20 | フィルタリング地域（南北最小緯度） |
| `USGS_REGION_LAT_MAX` | 50 | フィルタリング地域（南北最大緯度） |
| `USGS_REGION_LON_MIN` | 120 | フィルタリング地域（東西最小経度） |
| `USGS_REGION_LON_MAX` | 180 | フィルタリング地域（東西最大経度） |
| `USGS_NOTIFICATION_COOLDOWN` | 300 | 重複通知防止クールダウン（秒、5分） |
