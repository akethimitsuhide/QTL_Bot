...
### GIS地図描画機能（試験導入、2026-09-13〜。津波対応・自動ズーム・視認性向上は2026-09-14〜、遠地地震（国土地理院タイル重ね合わせ）対応は2026-09-15〜、観測点拡大表示・重なり順・気象庁形式の津波通知対応は2026-09-16〜、震度速報の区域アイコン・不透明色塗り・観測点マーカー拡大も2026-09-16〜）
...
投影は緯度経度→ピクセルの簡易正距円筒図法（表示範囲の中心緯度でcos補正）で、`shapely`/`cartopy`/`matplotlib`等の追加依存は使わない。GeoJSONのパース結果（区域名→生の緯度経度リング＋bbox）はプロセス内で一度だけ行いメモリ上に保持する（ピクセルへの投影は表示範囲ごとに変わるため描画のたびに行うが、900×900px相当の描画で1回あたり数十〜100ms程度）。`GIS_MAP_ENABLE=false`または外部データ未取得の場合は各描画関数が`None`を返し、画像添付自体が省略される（通知本体の送信は妨げない）。
...
**最終更新**: 2026-09-16（**不具合修正**：気象庁配信の津波予報・警報（`notify_tsunami_forecast`）・津波観測情報（`notify_tsunami_observation`）にGIS地図描画の配線が入っておらず地図が添付されない不具合を修正（P2P経由の`notify_tsunami`にしか配線していなかった。実機テストで発覚）／観測点マーカーの表示範囲を、震度3以上の観測点があればその周辺だけに拡大表示するよう変更（無ければ従来通り全観測点を含める）／観測点マーカーが重なる場合、震度が大きいものほど手前（前面）に描画されるよう描画順を震度コード昇順に変更／震度の塗りつぶしを半透明(150/255)から不透明（設定色そのまま）に変更し、区域境界は塗り色より暗い輪郭線で確保するよう変更（「色が薄い」との指摘のため）／震度速報（ScalePrompt）時、区域塗りに加えて各区域中央に震度ラベル付きの大きめアイコン（28px）を追加／観測点マーカーを11px→16px→24pxに拡大（S/M/Lの比較サンプルを提示し、Lの24pxを採用）)
**対応 Python**: 3.11+