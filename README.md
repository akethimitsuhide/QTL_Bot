...
### GIS地図描画機能（試験導入、2026-09-13〜。津波対応・自動ズーム・視認性向上は2026-09-14〜、遠地地震対応は2026-09-15〜、観測点拡大表示・重なり順・気象庁形式の津波通知対応は2026-09-16〜、震度速報の区域アイコン・不透明色塗り・観測点マーカー拡大も2026-09-16〜、陸地／海の色分け・アンチエイリアス・震源マークデザイン変更・国境データ重ね合わせ・対応情報種別拡大は2026-09-17〜、USGS対応・長周期地震動の観測点マップ・通常表示の併記・Webダッシュボード連携は2026-09-18〜、遠地地震向けの地理院タイル廃止・独自ベクター地図への統一は2026-09-26〜）
気象庁のシェープファイル形式のGISデータ（有志によりGeoJSON化されたもの、CC-BY-4.0）を用いて、震源・震度分布・津波予報区等の地図画像を独自に描画する機能。旧・P2P地図画像添付機能の置き換え。データ取得・キャッシュ（簡略化国境データの生成も含む）は`core/gis_data.py`、地図描画（日本国内向け・震源が日本国外の遠地地震向けとも、2026-09-26からは同一の投影方式・独自ベクター地図に統一）は`core/gis_render.py`（Pillowのみ使用。新規の重量級依存は追加していない）、1通知に複数のGIS地図画像を添付する際のDiscord埋め込み組み立ては`core/gis_discord.py`が担当する。`EewCog`・`QuakeInfoCog`・`TsunamiCog`・`OtherInfoCog`（長周期地震動・顕著な地震の震源要素更新のお知らせ）・`UsgsCog`（2026-09-18追加）に統合済み。
...
**有効化**: `.env`で`GIS_MAP_ENABLE=true`にする（デフォルト無効。試験導入のためオプトイン方式）。有効化すると、各Cogロード時に必要な外部データが未取得であれば自動的にダウンロードし、遠地地震向けの簡略化国境データ（下記）もバックグラウンドスレッドで事前生成する（初回のみ。手動で強制再取得したい場合は`python3 bot.py --refresh_gis_data`）。
...
- `AreaForecastLocalE_GIS`（細分区域、194件。緊急地震速報（予報）・全地震情報の震度分布描画用。区域名はREGION_MAP変換前の生の名称と一致。2件はgeometryがnull＝境界データ未整備のため塗りつぶし対象外。遠地地震向けの地図でも、日本の輪郭を示す区域線としてこのデータを再利用する）
...
- `countries.geojson`（[datasets/geo-countries](https://github.com/datasets/geo-countries)、258カ国、日本を除く。遠地地震向けの地図で、日本以外の陸地を塗りつぶすために使う。2026-09-17追加）
- `countries_simplified.geojson`（上記`countries.geojson`から`core/gis_data.py`が生成する派生キャッシュ。**2026-09-26追加**。元データは約14MB・頂点数約55万と非常に大きく、そのまま描画やWebダッシュボード配信に使うのは重いため、Ramer-Douglas-Peuckerアルゴリズム〈折れ線の間引き、追加の依存ライブラリなしで実装〉で頂点数を約4万・ファイルサイズを約0.6〜0.7MBまで間引いたもの。`core.gis_render.render_overseas_map`・Webダッシュボードの`GET /status/gis_countries`の両方が共用する。`--refresh_gis_data`で元データを再取得した場合は自動的に無効化され、次回アクセス時に作り直される）

いずれも[Ichihai1415/JMA-GIS-GeoJSON](https://github.com/Ichihai1415/JMA-GIS-GeoJSON)（GeoJSON3種）・[datasets/geo-countries](https://github.com/datasets/geo-countries)（countries.geojson）・気象庁公式（stations.json）から取得する。`countries_simplified.geojson`のみ外部取得ではなく、上記`countries.geojson`から本Bot自身が生成する派生データ。

> 2026-09-25以前から運用している場合、`GIS_MAP_DATA_DIR/tiles/`配下に国土地理院タイルのキャッシュ（PNG画像多数）が残っている可能性があるが、地理院タイル廃止（2026-09-26）に伴い参照されなくなったため、ディスク容量を空けたい場合は手動で削除して構わない（自動削除はしていない。稼働継続に支障はない）。
...
- USGS地震情報（2026-09-18追加）: USGSのGeoJSON標準形式（`[経度,緯度,深さ]`）の座標をそのまま使用。震源が日本国内／近海か国外かで通常のベクター地図・遠地地震向けの地図を自動的に切り替える
...
> **【2026-09-26 地理院タイルの使用を廃止】** それまでは国土地理院の淡色地図タイル（`https://cyberjapandata.gsi.go.jp/xyz/pale/{z}/{x}/{y}.png`）を背景に、世界の国境データ・区域境界線・震源のバツ印を重ね描きする方式（`core/gis_tile_render.py`）だったが、地理院タイルサーバー側の負荷・大規模地震時のアクセス集中への配慮から使用を取りやめた。日本国内向けの地図（`render_shindo_map`等）と同じ「GeoJSONのみ・ネットワークI/Oなし」の投影方式・アンチエイリアス処理に統一し、`core/gis_tile_render.py`は削除した。これに伴い、国土地理院コンテンツ利用規約に基づく出典表示（「出典：国土地理院…」）も不要になった。実際の地形の精密さではタイル画像に劣るが、震源のおおよその位置（どの国・地域か、日本との位置関係）を伝える目的には十分と判断している。旧実装がタイル取得のためのHTTPフェッチを必要としていた（`session`引数を取るasync関数だった）のに対し、`render_overseas_map()`はネットワークI/O不要の同期関数になった。
>
> 表示範囲は「日本全体 ∪ 震源」を収める範囲（旧タイル方式と同じ、余白比率15%）。日付変更線をまたぐ震源（南米・北米西岸等、太平洋を挟んで日本の反対側にある場合）でも、地球を逆回りした遠い側ではなく太平洋を挟んだ近い側の範囲を選ぶ（震源の経度を+360／-360シフトして候補を作り、経度方向の幅が最小になる方を採用）。**2026-09-26のベクター地図化にあたり新たに発見・修正した不具合**として、震源のバツ印だけでなく、国境データ側（`countries_simplified.geojson`）の各国ポリゴンの経度も、この表示範囲に合わせて同じ±360シフトを適用しないと、対象国が表示範囲の座標系からズレて描画されない（震源のバツ印だけが海上に浮いて見える）ことが実機検証前のシミュレーションで判明したため、`_pick_lon_shift()`で国ごとに最適なシフト量を選んでから描画するよう対応済み。

**自動ズーム（2026-09-14追加）**: 描画対象（塗りつぶし区域・観測点・震源・津波予報区の沿岸線）の緯度経度から表示範囲を動的に計算し、対象が日本の一部にとどまる場合はその周辺だけを拡大して描画する（余白は表示範囲の35%、最小表示範囲は2度四方を確保）。対象が広範囲、または該当データが無い場合は日本全体表示にフォールバックする。表示範囲外の区域境界線は描画自体をスキップし、Raspberry Pi での負荷を抑えている（遠地地震向けの地図は表示範囲の算出方法自体が別ロジックのため対象外。上記参照）。

**陸地／海の色分け・アンチエイリアス（2026-09-17追加）**: 従来は「塗られていない区域＝背景と同じ薄灰色」だったため日本列島の形そのものが把握しにくかったが、区域データを陸地色（クリーム色）で塗り、背景を海色（薄い水色）にすることで列島の形をひと目で分かるようにした。加えて、実際のサイズの2倍でキャンバスに描画してから最後にLANCZOSで縮小する簡易アンチエイリアス（スーパーサンプリング）により、県境・海岸線のギザつきを滑らかにしている。**2026-09-26**: それまでは遠地地震向けの地図（国土地理院タイル重ね合わせ）だけ投影方式がWeb Mercatorで異なるためこの処理を適用できなかったが、地理院タイル廃止により`render_overseas_map`も他の地図と同じ独自ベクター地図になったため、この陸地／海の色分け・アンチエイリアスも含め、描画処理そのものを共通化した（`_draw_land_fill`・`_draw_country_fill`・`_supersampled`等はいずれも`render_eew_warn_map`/`render_shindo_map`/`render_tsunami_map`/`render_overseas_map`の全てで共通利用）。
...
- **地震情報履歴の地図・表閲覧（2026-09-06追加、2026-09-09にEEW対応・日付/震度絞り込み・月次件数グラフ対応、2026-09-18にGIS区域境界線オーバーレイ対応、2026-09-26に地理院タイル廃止・マーカー表示改善）**: `GET /quake_map` で、地震情報（P2P地震情報 code=551）・EEW（緊急地震速報）の通知履歴を地図（Leaflet、震度に応じた色分け・規模に応じた大きさのマーカー）と表の両方で、ページ内の種別セレクタで切り替えながら閲覧できる。発生日（から〜まで）・最大震度でのフィルタ、月次件数の棒グラフ（Chart.js、フィルタ適用後の件数を表示）も備える。データ取得元は `GET /status/quake_history` / `GET /status/eew_history`（`?limit=N` で件数を絞り込み可能。省略時は `QUAKE_HISTORY_DEFAULT_LIMIT`）で、`core/quake_history_log.py` / `core/eew_history_log.py` が qtlbot.log（ローテーション含む全世代）をスキャンして復元した内容をそのまま返す。
  - **背景地図（2026-09-26変更）**: 従来は国土地理院タイルを背景に使っていたが、地理院タイルサーバー側の負荷・大規模地震時のアクセス集中への配慮から使用を取りやめた。代わりに、GIS地図描画機能でキャッシュ済みの世界の国境データ（簡略化版、`GET /status/gis_countries`）・日本の細分区域データ（`GET /status/gis_local_areas`）を陸地色で塗りつぶす、タイルサーバーに依存しない簡易ベースマップに切り替えた（いずれも`GIS_MAP_ENABLE=false`またはキャッシュ未取得時は404を返し、その場合は海色の背景のみの地図として動作する）。震源マーカーより必ず背面に描画されるよう、専用のLeafletペイン（`landFillPane`）に配置している
  - **マーカー表示の改善（2026-09-26追加）**: 円マーカーの半径をマグニチュードに応じて可変にした（M3.0〜M8.0を半径5px〜22pxへ線形マッピング）。また、最大震度が大きい地震ほど後から描画することで、地図上でマーカー同士が重なった場合に震度の大きい方が常に手前（前面）に表示されるようにした（震度が同じ場合はマグニチュードが大きい方を優先）
  - 詳細は下記「地震情報履歴（qtlbot.logベース）」参照
- `!status` コマンド（管理者専用）
...
└── core/
    ...
    │                                 対応。2026-09-26に地理院タイル廃止に伴いrender_overseas_map
    │                                 〈旧core/gis_tile_render.py〉を統合、全ての地図が同じ
    │                                 投影方式・描画処理を共有するようになった）
