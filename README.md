...
### GIS地図描画機能（試験導入、2026-09-13〜。津波対応・自動ズーム・視認性向上は2026-09-14〜、遠地地震（国土地理院タイル重ね合わせ）対応は2026-09-15〜、観測点拡大表示・重なり順・気象庁形式の津波通知対応は2026-09-16〜、震度速報の区域アイコン・不透明色塗り・観測点マーカー拡大も2026-09-16〜、陸地／海の色分け・アンチエイリアス・震源マークデザイン変更・国境データ重ね合わせ・対応情報種別拡大は2026-09-17〜）
気象庁のシェープファイル形式のGISデータ（有志によりGeoJSON化されたもの、CC-BY-4.0）を用いて、震源・震度分布・津波予報区等の地図画像を独自に描画する機能。旧・P2P地図画像添付機能の置き換え。データ取得・キャッシュは`core/gis_data.py`、日本国内向けの描画は`core/gis_render.py`（Pillowのみ使用。新規の重量級依存は追加していない）、震源が日本国外の場合の国土地理院タイル重ね合わせは`core/gis_tile_render.py`が担当する。`EewCog`・`QuakeInfoCog`・`TsunamiCog`・`OtherInfoCog`（長周期地震動・顕著な地震の震源要素更新のお知らせ）に統合済み。
...
- 地震情報（震度速報以外で観測点情報がある場合）: 観測点ごとに震度を表す短いラベル（"1"〜"7"、5弱/5強は"5-"/"5+"、6弱/6強は"6-"/"6+"）付きの色付き四角マーカー（24px、2026-09-16に11px→16px→24pxへ拡大。S/M/Lの比較サンプルを提示し、Lの24pxを採用）を配置。震度3以上の観測点がある場合はその周辺だけを拡大表示し（無ければ従来通り全観測点を表示範囲に含める）、観測点同士が重なる場合は震度が大きいものほど手前（前面）に描画する（2026-09-16追加）
...
- 長周期地震動に関する観測情報・顕著な地震の震源要素更新のお知らせ（2026-09-17追加）: いずれもJMAの`Coordinate`/`Coordinate_WGS`形式の座標文字列（`core.helpers.parse_jma_coordinate()`でパース）から震源の緯度経度を取り出し、震源のバツ印のみ描画する（長周期地震動階級はSHINDO_COLORSとは別スケールのため地域色分けは行わない）
...
- **地震情報履歴の地図・表閲覧（2026-09-06追加、2026-09-09にEEW対応・日付/震度絞り込み・月次件数グラフ対応）**: `GET /quake_map` で、地震情報（P2P地震情報 code=551）・EEW（緊急地震速報）の通知履歴を地図（Leaflet + 国土地理院タイル、震度に応じた色分けマーカー）と表の両方で、ページ内の種別セレクタで切り替えながら閲覧できる。発生日（から〜まで）・最大震度でのフィルタ、月次件数の棒グラフ（Chart.js、フィルタ適用後の件数を表示）も備える。データ取得元は `GET /status/quake_history` / `GET /status/eew_history`（`?limit=N` で件数を絞り込み可能。省略時は `QUAKE_HISTORY_DEFAULT_LIMIT`）で、`core/quake_history_log.py` / `core/eew_history_log.py` が qtlbot.log（ローテーション含む全世代）をスキャンして復元した内容をそのまま返す。詳細は下記「地震情報履歴（qtlbot.logベース）」参照
- `!status` コマンド（管理者専用）
...
**最終更新**: 2026-09-17（GIS地図描画（`core/gis_render.py`の独自ベクター地図）に陸地／海の色分け（陸地はクリーム色、海は水色）とアンチエイリアス（2倍スーパーサンプリング→LANCZOS縮小）を追加／震源のバツ印を「白い丸フチ」方式から「赤バツ印（前面）＋一回り大きい白バツ印（背面、縁取り）」方式に変更（`core/gis_tile_render.py`の海外震源マップにも自動反映）／長周期地震動に関する観測情報・顕著な地震の震源要素更新のお知らせ（`cogs/other.py`）にGIS地図描画を追加（`core.helpers.parse_jma_coordinate()`でJMAの座標文字列から緯度経度を抽出。津波観測に関する情報・沖合の津波観測に関する情報は既に対応済みだったことを確認）／遠地地震向けの国土地理院タイル重ね合わせに、世界の国境データ（`countries.geojson`、日本を除く258カ国）を背景の下地として追加し、国土地理院タイルが薄い海外の地域でも陸地／海の判別ができるようにした（台湾・チリ震源でのテストで確認済み）)
**対応 Python**: 3.11+