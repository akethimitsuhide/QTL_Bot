...
- **地震情報**: P2P 地震情報の API からの確報情報
  ...
  - **P2P地震情報のエリアの`scaleTo=99`の扱い（2026-09-22修正、2026-09-24にPLUM法の扱いを再訂正）**: `scaleTo=99`は、`scaleFrom`（下限）だけが判明していて上限が定まらない状態（気象庁電文の`To="over"`相当と見ている。この対応関係はP2P地震情報の公開API仕様書等で一次情報を確認できておらず、既存コードの前提とコード全体の挙動から推測したもの。ここは断定を避ける）で、従来は一律「震度7」として扱っていたため、非PLUM法の発表で全体の予想最大震度（`MaxIntensity`）や地域ごとの内訳が実態と無関係に「7以上」等と表示される不具合があった（2026-09-22修正）。**PLUM法（`isAssumption`）のエリアはこの「以上」表記の対象から除外している（2026-09-24修正）**: 当初は非PLUM法・PLUM法の別なく`scaleTo=99`を一律「◯◯以上」と表示するよう修正していたが、Wolfx側はPLUM法の推定震度を「以上」を付けずに（例:「震度5弱程度」）表示しており、P2P地震情報側もそれに合わせるべきとの指摘を受けて、PLUM法のエリアは`scaleTo=99`を`scaleFrom`が不明な場合と同様に無視し、下限（`scaleFrom`＝PLUM法自体の推定値）をそのまま「震度X程度」として表示するよう修正した（非PLUM法の発表では、従来通り`scaleTo=99`のエリアを`core.eew_convert.OPEN_UPPER_LABEL`として扱い「◯◯以上」と表示する）。Wolfx形式（`Shindo2`が電文由来の`"over"`表記のケース）も同様に非PLUM法のみ「以上」表記の対象とする（`core/helpers.py`の`shindo_code_from_max_label`で、Embedの色・警戒喚起文・音声判定・地図描画のすべてが「◯◯以上」表記を取りこぼさないようにしている）
...
- 日付変更線をまたぐ震源（南米・北米西岸等、太平洋を挟んで日本の反対側にある場合）でも、地球を逆回りした遠い側ではなく太平洋を挟んだ近い側の範囲を正しく選択する（**2026-09-25修正**：表示範囲〈bbox〉の算出では震源の経度を+360／-360シフトして近い側を選ぶが、震源のバツ印・出典表示の位置を求める座標変換に、このシフトを反映していない元の経度をそのまま使っていたため、西経の震源〈北米・南米。例:コロンビア経度-74°〉でバツ印がキャンバス範囲外に描画され、表示されない不具合があった。バツ印・出典表示にも、bbox算出時と同じ経度シフトを適用するよう修正）
...
#### 1. Discord Bot トークン取得・自分のサーバーへの追加（初めての方向け）
Discord Developer Portal を触るのが初めてでも迷わないよう、Bot アプリケーションの作成から
自分のサーバー（自鯖）への追加までを一つずつ説明する。「権限がよく分からない」という
場合も、下記の通りに進めれば迷わないはず。

**0. 招待先のDiscordサーバーを用意する（すでに持っている場合はスキップ）**
- Discordアプリ（PC版・ブラウザ版どちらでも可）左端の「＋」ボタン→「オリジナルの作成」→
  「自分と友達のため」を選ぶと、自分がオーナー（＝全権限を持つ）のサーバーを作成できる。
- 通知を試すだけの検証用サーバーで構わない。本番用に既に持っているサーバーがあれば、
  そちらでも構わない（ただし、そのサーバーで自分が「サーバー管理」権限を持っている必要が
  ある。詳しくは手順5参照）。

**1. Botアプリケーションを作成する**
1. [Discord Developer Portal](https://discord.com/developers/applications) にアクセスし、
   Discordアカウントでログインする。
2. 右上の「**New Application**」をクリック。
3. アプリケーション名（例: `QTL_Bot`。後から変更可能）を入力し「**Create**」。
   → これでBotの「アプリケーション」が1つ作成される（まだサーバーには追加されていない）。

**2. Botトークンを取得する（取り扱い注意）**
1. 作成したアプリケーションのページで、左メニューから「**Bot**」を開く。
2. 「Token」欄の「**Reset Token**」（初回は「Copy」の場合もある）をクリックし、表示された
   トークンをコピーする。
   - **このトークンは`.env`のBOT_TOKENの値そのものであり、パスワードと同じ扱いが必要**。
     人に見せたり、GitHub等に誤ってコミットしたりしないこと（本リポジトリの`.gitignore`は
     `.env`自体を除外しているので、通常の使い方であれば誤コミットは防げる）。
   - トークンを紛失した・漏洩した疑いがある場合は、同じ「Reset Token」から再発行すれば、
     古いトークンは即座に無効化される（Botが動かなくなるので、再発行したら`.env`も
     更新すること）。

**3. 必要なIntent（Botが受け取れる情報の種類）を有効化する**
同じ「Bot」ページを下にスクロールし、「Privileged Gateway Intents」の中の以下2つを
**ON**にして、ページ下部の「Save Changes」を押す。
- **MESSAGE CONTENT INTENT**: メッセージの本文を読み取れるようにする。本Botの
  `!status`等のプレフィックスコマンドの認識に必要。
- **SERVER MEMBERS INTENT**: サーバーメンバー一覧を取得できるようにする。

**4. OAuth2 URLを発行し、自分のサーバーに招待する**
1. 左メニューから「**OAuth2**」→「**URL Generator**」を開く。
2. 「SCOPES」で「**bot**」にチェックを入れる。
3. チェックすると下に「BOT PERMISSIONS」が現れるので、「**Administrator**」に
   チェックを入れる（Botが動作に必要とする権限を個別に選ぶ方法もあるが、
   初めての場合は取りこぼしを避けるため一括でAdministratorを付与するのが簡単。
   検証用サーバーではなく共有サーバーに入れる場合は、サーバー管理者と相談のうえ
   権限範囲を絞ることを検討してもよい）。
4. ページ下部に生成された **URL** をコピーし、ブラウザの新しいタブで開く。
5. 「**サーバーに追加**」のプルダウンで、招待先のサーバー（手順0で用意したもの）を選択し
   「続ける」→ 権限確認画面で「**認証**」をクリックする。
   - **このプルダウンに自分のサーバーが出てこない場合**、そのサーバーで自分が
     「**サーバー管理（Manage Server）**」権限を持っていない（オーナーでない・
     管理者ロールを持っていない）ことが原因。自分がオーナーの検証用サーバー
     （手順0）であれば必ず表示される。
6. 認証が完了すると、Botがサーバーのメンバー一覧に（オフライン状態で）追加される。
   `.env`を設定して`python bot.py`を起動すると、オンラインになる。

以上でBotの追加は完了。次は`.env`の設定（手順2）に進む。
...
**チャンネルIDの確認方法（初めての場合）**: 通常、Discordのメニューにはチャンネル
IDをコピーする項目が出ない。以下の手順で表示させる必要がある。
1. Discordの設定（左下の歯車アイコン）→「**詳細設定**」→「**開発者モード**」をONにする。
2. 対象のテキストチャンネルを右クリック（スマホの場合は長押し）→「**IDをコピー**」を選択。
3. コピーされた数字（18〜19桁）をそのまま`.env`の該当項目に貼り付ける。
...
| `SCRATCHTTS_URL` | `https://synthesis-service.scratch.mit.edu/synth` | ScratchTTS のエンドポイント（`TTS_ENGINE=scratchtts` 時のみ使用） |
...
| `APM_OTLP_ENDPOINT` | `https://otlp-vaxila.mackerelio.com/v1/traces` | OTLP 送信先エンドポイント。`core/apm.py` はパスを追加結合しないため、変更する場合は末尾を必ず`/v1/traces`にすること（**2026-09-24修正**: 旧記載の`https://otlp-vmagent.mackerelio.com`は`.env.example`・コードの既定値と食い違っていた。Mackerel公式ドキュメントでの一次確認は未実施） |
