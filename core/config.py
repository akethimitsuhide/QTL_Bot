...
def _require_env(key: str) -> str:
    ...
    if not value:
        logger.critical(
            f"環境変数 {key} が設定されていません。.env を確認してください。"
        )
        raise SystemExit(1)
    return value
...
LOG_SUPPRESS_HTTP_SUCCESS = (
    os.getenv("LOG_SUPPRESS_HTTP_SUCCESS", "true").lower() == "true"
)
...
try:
    ...
    logger.critical(
        f"CHANNEL_ID は数値で指定してください（現在の値: {_channel_id_raw!r}）"
    )
    raise SystemExit(1)
...
if _raw_aquestalk:
    ...
    if not os.path.isfile(_raw_aquestalk):
        logger.warning(
            f"AQUESTALK_PATH のファイルが見つかりません: {_raw_aquestalk} → 音声機能を無効化します"
        )
        _raw_aquestalk = ""
    elif not os.access(_raw_aquestalk, os.X_OK):
        logger.warning(
            f"AQUESTALK_PATH に実行権限がありません: {_raw_aquestalk} → 音声機能を無効化します"
        )
...
def get_channel_id(*keys: str, default: int = 0) -> int:
    for key in keys:
        value = os.getenv(key)
        if value:
            return int(value)
...
# チャンネル ID 設定
# ===============================
EEW_CHANNEL_ID = get_channel_id("EEW_CHANNEL_ID", "CHANNEL_ID")
QUAKE_CHANNEL_ID = get_channel_id("QUAKE_CHANNEL_ID", "CHANNEL_ID")
TSUNAMI_CHANNEL_ID = get_channel_id("TSUNAMI_CHANNEL_ID", "CHANNEL_ID")
OTHER_CHANNEL_ID = get_channel_id("OTHER_CHANNEL_ID", "CHANNEL_ID")

P2P_EEW_CHANNEL_ID = get_channel_id(
    "P2P_EEW_CHANNEL_ID",
    "EEW_CHANNEL_ID",
    "CHANNEL_ID",
)

KYOSHIN_CHANNEL_ID = get_channel_id(
    "KYOSHIN_CHANNEL_ID",
    "OTHER_CHANNEL_ID",
    "CHANNEL_ID",
)

ADMIN_CHANNEL_ID = get_channel_id("ADMIN_CHANNEL_ID")
VOLCANO_CHANNEL_ID = get_channel_id("VOLCANO_CHANNEL_ID", "CHANNEL_ID")
USGS_CHANNEL_ID = get_channel_id(
    "USGS_CHANNEL_ID",
    "QUAKE_CHANNEL_ID",
    "CHANNEL_ID",
)
...
APM_OTLP_ENDPOINT = os.getenv(
    "APM_OTLP_ENDPOINT", "https://otlp-vaxila.mackerelio.com/v1/traces"
)
