...
# 揺れ色抽出のしきい値（HSVマスク処理）
# ===============================
# 「この震度相当」以上（テーブル上では概ね hue<=90°程度）を"揺れ候補色"とする。
# 参考資料の「黄色以上を抽出する」という考え方に基づく。
# これより弱い色（背景の青〜水色域）は、GIFノイズの温床であるため
# 最初から解析対象に含めない。
#
# 【2026-07-21 緩和の経緯】
# 当初は 1.0（気象庁震度階級で言う震度1相当）だったが、実際の地震
# （気象庁震度1、山梨県東部・富士五湖）で画像解析側の検知が
# 一切反応しなかった事例が発生した。気象庁震度階級は離散値、
# 防災科研リアルタイム震度は連続値であり、気象庁震度1の地震でも
# 各観測点のリアルタイム震度実数値は 0.5〜1.4 程度に分布しうる。
# ACTIVE_SHINDO_FLOOR=1.0 だと、その多くが最初のHSVマスク段階で
# 除外され、後段のクラスタリング・確定判定に一切到達しない
# （＝検知そのものが起きない）ことが主要因だったと判断し、0.5に緩和した。
#
# 【2026-07-21 追記】この後さらに0.2まで緩和する変更を試みたが、
# 単一観測点由来のGIF圧縮ノイズ（実震度0.2程度）まで解析対象に
# 含まれてしまい、他社製ソフトでは検知しないレベルのノイズで
# 誤検知が多発したため、0.5より下げない方針とする。
ACTIVE_SHINDO_FLOOR = float(os.getenv("KYOSHIN_ACTIVE_SHINDO_FLOOR", "0.5"))
...
class KyoshinImageAnalyzer:
    ...
    def analyze(self, image) -> list[GridCellReading]:
        ...
        【2026-08 補足】cogs/kyoshin_monitor.py の検知パイプラインは
        実観測点データ方式（core.kyoshin_stations）に移行済みのため、
        このクラスの用途は core.kyoshin_shared.estimate_max_shindo_from_image
        （EEW発表時トリガーの振動モニタ機能、画像全体の最大震度概算）
        のみになった。analyze_all() / build_station_grid()（旧・疑似
        グリッド観測点方式で使用）は呼び出し元が無くなったため削除した。
        """
        if image.mode != "RGB":
